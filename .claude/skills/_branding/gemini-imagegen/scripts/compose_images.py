...
def format_quota_error(instruction: str, image_paths: List[str]) -> str:
    """Format a user-friendly quota exhaustion message with fallback instructions.

    Args:
        instruction: The composition instruction that couldn't be processed
        image_paths: List of input image paths

    Returns:
        Formatted error message with instruction and fallback instructions
    """
    separator = "━" * 60
    header = "=" * 60
...
2. Upload all {len(image_paths)} images listed above
3. Paste the instruction in the chat
...
def main():
    ...
    try:
        ...
        print(format_quota_error(args.instruction, args.images), file=sys.stderr)
        sys.exit(429)

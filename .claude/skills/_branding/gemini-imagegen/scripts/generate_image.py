...
def format_quota_error(prompt: str) -> str:
    """Format a user-friendly quota exhaustion message with fallback instructions.

    Args:
        prompt: The optimized prompt that couldn't be generated

    Returns:
        Formatted error message with prompt and fallback instructions
    """
    separator = "━" * 60
    header = "=" * 60
...
3. The web interface has a SEPARATE quota from the API
...
def main():
    ...
    try:
        ...
        print(format_quota_error(args.prompt), file=sys.stderr)
        sys.exit(429)
